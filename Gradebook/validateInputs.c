...
int validateGradeInput() // Ref[1]/Ref[2]
{
    ...
    while (scanf_s("%d", &uInput) != 1 || uInput < GRADE_MIN || uInput > GRADE_MAX)
    {
        printf("\n");
...
int validateStudentSelection() //Ref[1] / Ref[2]
{
    ...
    while (scanf_s("%d", &uInput) != 1 || uInput < 0 || uInput > STUDENT_LIMIT)
    {
        printf("\n");
        printf("Invalid input. Select an integer from 0 - %d: ", STUDENT_LIMIT);
        while (getchar() != '\n'); // Clear input buffer. Otherwise will get infinite loop when invalid input. 
...
{
    int uInput;
    while (scanf_s("%d", &uInput) != 1 || uInput < 1 || uInput > MENU_TOTAL_OPTIONS)
    {
        printf("\n");
